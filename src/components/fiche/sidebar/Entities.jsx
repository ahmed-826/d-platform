      ...
      {Object.entries(groupedEntities).map(([category, entities]) => (
        <div key={category} className="mb-3">
          <div className="flex items-center mb-1">
            {category === "person" && (
              <Users size={14} className="mr-1 text-blue-500/70" />
            )}
            {category === "organization" && (
              <Building size={14} className="mr-1 text-blue-500/70" />
            )}
            {category === "location" && (
              <MapPin size={14} className="mr-1 text-blue-500/70" />
            )}
            <div className="text-xs font-medium uppercase tracking-wider text-gray-600/80">
              {category === "person"
                ? "Personnes"
                : category === "organization"
                ? "Organisations"
          ...
          <div className="ml-1">
            {entities.map((entity) => (
              <div key={entity.id} className="mb-2">
                <div
                  className="px-2 py-1 text-sm hover:bg-gray-100/80 rounded cursor-pointer transition-colors"
                  onClick={() => toggleEntityDescription(entity.id)}
                >
                  {entity.name}
                </div>
                {expandedEntityIds.includes(entity.id) && (
                  <div className="ml-2 mt-1 px-3 py-2 text-xs bg-gray-100/40 rounded-md border-l-2 border-blue-500/30 animate-fade-in">
                    {entity.description}
                  </div>
                )}
              </div>
